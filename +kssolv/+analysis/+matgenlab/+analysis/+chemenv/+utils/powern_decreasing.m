function value=powern_decreasing(input,varargin)
%POWERN_DECREASING Finite polynomial decrease.
[edges,degree]=options(varargin,2,"nn");
if ~isempty(edges)
    input=kssolv.analysis.matgenlab.analysis.chemenv.utils. ...
        scale_and_clamp(input,edges(1),edges(2),0,1);
end
value=(input-1).^degree/(-1)^degree;
end
