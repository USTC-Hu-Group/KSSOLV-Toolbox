classdef VolumetricData < kssolv.analysis.matgenlab.util.MSONable
    ...
    properties
        ...
        grid_origin (1,3) double = [0, 0, 0]
        voxel_vectors double = zeros(0, 3)
        grid_periodic (1,3) logical = [true, true, true]
        grid_sampling (1,1) string = "cell-periodic"
        channel_labels (1,1) struct = struct()
    end
    ...
    methods
        function obj = VolumetricData(structure, data, options)
            ...
            dimensions = size(data.total);
            if numel(dimensions) < 3, dimensions(3) = 1; end
            obj.dim = dimensions(1:3);
            obj.ngridpts = prod(obj.dim);
            obj.is_spin_polarized = numel(fieldnames(data)) >= 2;
            obj.is_soc = numel(fieldnames(data)) >= 4;
            obj.grid_origin = options.grid_origin;
            obj.grid_periodic = options.grid_periodic;
            obj.grid_sampling = options.grid_sampling;
            obj.channel_labels = options.channel_labels;
            if isempty(options.voxel_vectors)
                obj.voxel_vectors = ...
                    double(structure.lattice.matrix) ./ obj.dim.';
            elseif ~isequal(size(options.voxel_vectors), [3, 3])
                error("KSSOLV:Matgenlab:VolumetricData:VoxelVectors", ...
                    "voxel_vectors must have shape (3, 3).");
            else
                obj.voxel_vectors = double(options.voxel_vectors);
            end
        end
        ...
        function to_cube(obj, filename, comment)
            ...
            origin = obj.grid_origin * conversion;
            lines(3) = sprintf("\t %d %.15g %.15g %.15g", ...
                obj.structure.num_sites, origin);
            for axis = 1:3
                voxel = obj.voxel_vectors(axis, :) * conversion;
    ...
    methods (Static)
        ...
        function obj = from_cube(filename)
            ...
            signedAtomCount = round(header(1));
            numberAtoms = abs(signedAtomCount);
            dimensions = zeros(1, 3);
            voxels = zeros(3, 3);
            signedDimensions = zeros(1, 3);
            for axis = 1:3
                ...
                signedDimensions(axis) = round(row(1));
                dimensions(axis) = abs(signedDimensions(axis));
                if dimensions(axis) == 0
                    error("KSSOLV:Matgenlab:VolumetricData:CubeVoxel", ...
                        "Cube grid dimensions must be nonzero.");
                end
                voxels(axis, :) = row(2:4);
            end
            kssolv.analysis.matgenlab.io.vasp.VolumetricData. ...
                validateGridCount(dimensions, 1);
            usesAngstrom = signedDimensions < 0;
            if any(usesAngstrom) && ~all(usesAngstrom)
                error("KSSOLV:Matgenlab:VolumetricData:CubeUnits", ...
                    "Cube voxel-count signs must use one consistent unit.");
            end
            if all(usesAngstrom)
                conversion = 1;
            else
            ...
            for index = 1:numberAtoms
                ...
                species{index} = ...
                    kssolv.analysis.matgenlab.core.Element.from_Z( ...
                        round(row(1)));
                coordinates(index, :) = row(3:5) * conversion;
            end
            lattice = voxels .* dimensions.';
            structure = kssolv.analysis.matgenlab.core.Structure( ...
                lattice, species, coordinates, ...
                coords_are_cartesian = true);
            payload = sscanf(strjoin(lines(7 + numberAtoms:end), " "), "%f");
            componentCount = 1;
            componentIds = 1;
            if signedAtomCount < 0
                if isempty(payload)
                    error("KSSOLV:Matgenlab:VolumetricData:CubeDatasets", ...
                        "Cube dataset identifiers are missing.");
                end
                componentCount = round(payload(1));
                if componentCount < 1 || numel(payload) < componentCount + 1
                    error("KSSOLV:Matgenlab:VolumetricData:CubeDatasets", ...
                        "Invalid Cube dataset identifier block.");
                end
                componentIds = round(payload(2:componentCount + 1)).';
                payload = payload(componentCount + 2:end);
            elseif numel(header) >= 5
                componentCount = round(header(5));
                if componentCount < 1
                    error("KSSOLV:Matgenlab:VolumetricData:CubeDatasets", ...
                        "Cube NVAL must be a positive integer.");
                end
                componentIds = 1:componentCount;
            end
            kssolv.analysis.matgenlab.io.vasp.VolumetricData. ...
                validateGridCount(dimensions, componentCount);
            values = payload;
            expected = prod(dimensions) * componentCount;
            ...
            ordered = reshape(values, [componentCount, dimensions(3), ...
                dimensions(2), dimensions(1)]);
            data = struct();
            labels = struct();
            for component = 1:componentCount
                grid = permute(ordered(component, :, :, :), [4, 3, 2, 1]);
                grid = reshape(grid, dimensions);
                if component == 1
                    key = "total";
                else
                    key = "dataset_" + string(componentIds(component));
                    key = string(matlab.lang.makeValidName(key));
                end
                data.(key) = grid;
                labels.(key) = string(componentIds(component));
            end
            obj = kssolv.analysis.matgenlab.io.vasp.VolumetricData( ...
                structure, data, grid_origin = origin, ...
                voxel_vectors = voxels, ...
                grid_periodic = [false, false, false], ...
                grid_sampling = "point-inclusive", ...
        ...
        function [poscar, data, data_aug] = parse_file(filename)
            ...
            while lineIndex <= numel(lines)
                ...
                elseif contains(line, ".")
                    ...
                    kssolv.analysis.matgenlab.io.vasp.VolumetricData. ...
                        validateGridCount(dims, numel(datasets) + 1);
                    [payload, lineIndex] = ...
                        kssolv.analysis.matgenlab.io.vasp.VolumetricData. ...
            ...
            if ~any(numel(datasets) == [1, 2, 4])
                error("KSSOLV:Matgenlab:VolumetricData:Components", ...
                    ["VASP volumetric files require 1, 2, or 4 grids; " ...
                    "found %d."], numel(datasets));
            end
            if numel(datasets) == 4
    ...
    methods (Static, Access = private)
        function count = validateGridCount(dimensions, componentCount)
            dimensions = double(reshape(dimensions, 1, []));
            if numel(dimensions) ~= 3 || ...
                    any(~isfinite(dimensions)) || ...
                    any(dimensions < 1) || ...
                    any(dimensions ~= fix(dimensions))
                error("KSSOLV:Matgenlab:VolumetricData:GridLimit", ...
            ...
            for dimension = dimensions
                if count > flintmax / dimension
                    error("KSSOLV:Matgenlab:VolumetricData:GridLimit", ...
                        "Grid dimension product exceeds the exact range.");
                end
            ...
            if count > maximumVoxels || ...
                ...
                error("KSSOLV:Matgenlab:VolumetricData:GridLimit", ...
                    "Grid requests %.3g voxels, %d components, and " + ...
                    "%.3g values; limits are %.3g voxels, %d " + ...
                    "components, and %.3g values. " + ...
                    "Use a smaller grid or precomputed LOD.", ...
                    count, componentCount, count * componentCount, ...
                    maximumVoxels, maximumComponents, maximumValues);
            end
        end
