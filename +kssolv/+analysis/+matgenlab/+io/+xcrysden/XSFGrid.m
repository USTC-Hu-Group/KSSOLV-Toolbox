classdef XSFGrid < kssolv.analysis.matgenlab.util.MSONable
    %XSFGRID Static two- or three-dimensional XSF scalar-grid data.
    ...
    methods
        function obj = XSFGrid(data, lattice, origin, options)
            arguments
                data double
                lattice double
                origin double
            ...
            if ~isempty(obj.labels) && numel(obj.labels) ~= size(data, 1)
                error("KSSOLV:Matgenlab:XSFGrid:LabelCount", ...
        ...
        function value = get.ndim(obj)
            value = ndims(obj.data) - 1;
        end

        function value = as_dict(obj)
            value = struct();
            value.x_module = "pymatgen.io.xcrysden";
            value.x_class = "XSFGrid";
            value.data = obj.data;
    ...
    methods (Static)
        function obj = from_dict(value)
            ...
            obj = kssolv.analysis.matgenlab.io.xcrysden.XSFGrid( ...
