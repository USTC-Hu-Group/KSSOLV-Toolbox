classdef AtomicLatticeFunctionalTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function crystalEditsEstimateAndEnforceSymmetryChoice(testCase)
            crystal=kssolv.analysis.matgenlab.core.Structure. ...
                from_spacegroup("Fm-3m",eye(3)*3.61,"Cu",[0,0,0]);
            preserved=kssolv.modeling.CommandExecutor.execute(crystal, ...
                "substitute_atoms",struct("indices",1,"species","Ni", ...
                "symmetryMode","preserve"));
            testCase.verifyEqual( ...
                preserved.analysis.symmetryImpact.affectedCount,4);
            testCase.verifyEqual( ...
                preserved.analysis.symmetryImpact.resultNumber,225);
            testCase.verifyTrue(all(arrayfun( ...
                @(index)preserved.model(index).species_string=="Ni",1:4)));

            broken=kssolv.modeling.CommandExecutor.execute(crystal, ...
                "substitute_atoms",struct("indices",1,"species","Ni", ...
                "symmetryMode","break"));
            testCase.verifyEqual( ...
                broken.analysis.symmetryImpact.affectedCount,4);
            testCase.verifyEqual(sum(arrayfun( ...
                @(index)broken.model(index).species_string=="Ni",1:4)),1);
            testCase.verifyNotEqual( ...
                broken.analysis.symmetryImpact.resultNumber,225);

            moved=kssolv.modeling.CommandExecutor.execute(crystal, ...
                "move_atoms",struct("indices",1, ...
                "coordinates",[.1,.2,.3],"cartesian",false, ...
                "symmetryMode","preserve"));
            testCase.verifyEqual(moved.analysis.symmetryImpact.affectedCount,4);
            testCase.verifyEqual(moved.analysis.symmetryImpact.resultNumber,225);
        end
    end
