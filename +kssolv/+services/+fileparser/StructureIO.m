classdef StructureIO < handle
    ...
    methods
        function this = StructureIO(filePath, format)
            ...
            try
                ...
                if isa(this.MatgenlabObject, ...
                    ...
                    this.MatgenlabObject.properties.input_format = ...
                        this.fileType;
                    this.MatgenlabObject.properties.source_file = ...
                        this.filePath;
                end
        ...
        function updateMatgenlabObject(this, value)
            %UPDATEMATGENLABOBJECT Replace the in-memory model after editing.
            if ~(isa(value, ...
                    "kssolv.analysis.matgenlab.core.IStructure") || ...
                    isa(value, ...
                    "kssolv.analysis.matgenlab.core.IMolecule"))
                error("KSSOLV:FileParser:StructureIO:MatgenlabType", ...
                    "Expected a matgenlab structure or molecule, received '%s'.", ...
                    class(value));
            end
            this.MatgenlabObject = value.copy();
            this.KSSOLVObjectCache = [];
            this.KSSOLVSetupObject = ...
                kssolv.services.fileparser.StructureIO.toSetupObject( ...
    ...
    methods (Static, Access = private)
        function [value, format] = readMatgenlabObject(filePath, requestedFormat)
            ...
            if kssolv.services.fileparser.StructureIO.canRead( ...
                    moleculeHandler)
                try
                    moleculeFormat = string(moleculeHandler.name);
                    if moleculeFormat == "xyz"
                        frames = kssolv.analysis.matgenlab.io.xyz.XYZ. ...
                            from_file(filePath).all_molecules;
                        value = frames{1}.copy();
                        value.properties.frame_index = 1;
                        value.properties.frame_count = numel(frames);
                    elseif any(moleculeFormat == [ ...
                            "pdb", "mol", "mdl", "sdf", "sd", ...
                            "mol2", "ml2", "sy2", "cml", "mrv"])
                        adaptors = ...
                            kssolv.analysis.matgenlab.io.babel. ...
                            BabelMolAdaptor.from_file( ...
                            filePath, moleculeFormat, true);
                        value = adaptors{1}.pymatgen_mol;
                        value.properties.frame_index = 1;
                        value.properties.frame_count = numel(adaptors);
                    elseif ~isempty(moleculeHandler.read_file)
