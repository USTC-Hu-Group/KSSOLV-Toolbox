classdef MoleculeSceneBuilderTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function formulaReportsTheAtomsInTheEditableDocument(testCase)
            oxygen = kssolv.analysis.matgenlab.core.Molecule( ...
                "O", [0, 0, 0], charge_spin_check = false);
            oxygenScene = ...
                kssolv.ui.scene.atomic.MoleculeSceneBuilder.build( ...
                oxygen, includeConnectivity = false);
            testCase.verifyEqual(oxygenScene.molecule.formula, "O");
            ...
            waterScene = ...
                kssolv.ui.scene.atomic.MoleculeSceneBuilder.build( ...
                water, includeConnectivity = false);
            testCase.verifyEqual(waterScene.molecule.formula, "H2O");
        end

        function molecularDiagnosticsReachWarningStackWithStableSites(testCase)
            molecule = kssolv.analysis.matgenlab.core.Molecule( ...
                ["C", "H", "H", "H", "H", "H"], ...
                [0, 0, 0; .1, 0, 0; -.9, 0, 0; 0, .9, 0; ...
                0, -.9, 0; 0, 0, .9], charge_spin_check = false);
            molecule.properties.topology = struct( ...
                "bonds", [1, 2, 1; 1, 3, 1; 1, 4, 1; 1, 5, 1; 1, 6, 1], ...
                "origin", "source");
            scene = kssolv.ui.scene.atomic.MoleculeSceneBuilder.build(molecule);
            codes = string({scene.warnings.code});
            collision = find(codes == "ATOM_COLLISION", 1);
            valence = find(codes == "ILLEGAL_VALENCE", 1);
            testCase.verifyNotEmpty(collision);
            testCase.verifyNotEmpty(valence);
            testCase.verifyEqual(scene.warnings(collision).siteIndices, [0, 1]);
            testCase.verifyEqual(scene.warnings(valence).siteIndices, 0);
        end
        ...
        function singletonTransportCollectionsRemainArrays(testCase)
            ...
            scene.warnings = ...
                ...
                "SINGLE_SITE", "single", "warning", 1);
            encoded=string(jsonencode( ...
                kssolv.ui.scene.atomic.CrystalSceneSerializer. ...
                transportScene(scene)));
        ...
        function modelingTransportIncludesTraceableConstructionGeometry(testCase)
            molecule = kssolv.analysis.matgenlab.core.Molecule( ...
                ["C", "O"], [0, 0, 0; 1.43, 0, 0]);
            scene = kssolv.ui.scene.atomic.MoleculeSceneBuilder.build( ...
                molecule, includeConnectivity = false);
            modeling = kssolv.ui.scene.atomic.CrystalSceneSerializer. ...
                modeling(scene);
            encoded = string(jsonencode(modeling));
            testCase.verifyTrue(contains(encoded, ...
                '"constructionBonds":['));
            testCase.verifyTrue(contains(encoded, ...
                '"parameterSet":"kssolv-generic-mm-parameters-v2"'));
            parameters = [modeling.constructionBonds{:}];
            selected = parameters( ...
                string({parameters.firstElement}) == "C" & ...
                string({parameters.secondElement}) == "O" & ...
                [parameters.bondOrder] == 1);
            testCase.verifyEqual(selected.value, 1.43, ...
                "AbsTol", 1e-12);
            testCase.verifyEqual(selected.source, ...
                "frozen-pymatgen-bond-lengths");
            testCase.verifyFalse(selected.fallback);
        end
    end
end
